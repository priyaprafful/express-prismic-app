...
class Client {
  ...
  async getByUIDs(documentType, uids, params) {
    return await this.get(appendPredicates(typePredicate(documentType), _in(`my.${documentType}.uid`, uids))(params));
  }
  async getAllByUIDs(documentType, uids, params) {
    return await this.getAll(appendPredicates(typePredicate(documentType), _in(`my.${documentType}.uid`, uids))(params));
  }
  ...
  async getIntegrationFieldsRef() {
    const repository = await this.getCachedRepository();
    return repository.integrationFieldsRef || void 0;
  }
  async getResolvedIntegrationFieldsRef() {
